...
exports.create = function (req, res) {
  ...
  postModel.save(function (err, data) {
    if (err) {
      ...
      res
        .status(500)
        .send({ message: "An error occurred while adding blog post." });
    } else {
      console.log(data);
...
exports.findAll = function (req, res) {
  Post.find({}, function (err, post) {
    if (err) {
      ...
      res
        .status(500)
        .send({ message: "An error occurred while retrieving blog posts." });
    } else {
...
exports.updatePost = function (req, res) {
  ...
  Post.findOneAndUpdate(
    ...
    function (err, doc) {
      if (err) {
        console.log("Something wrong with updating post!");
